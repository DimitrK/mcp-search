...
- `src/core/search/googleClient.ts` uses `rate-limiter-flexible` to pace Google Custom Search requests by the configured `CONCURRENCY`.
- Embedding requests are batched by `EMBEDDING_BATCH_SIZE` in `src/core/vector/providers/httpEmbeddingProvider.ts`.
- `SimilaritySearchManager.searchMultiple()` processes query batches up to `CONCURRENCY`.
...
**File Location**: `{DATA_DIR}/db/mcp-{sanitized-model-name}.duckdb`
