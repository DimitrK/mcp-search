...
export const MCP_TOOL_DESCRIPTIONS = {
  WEB_SEARCH:
    'Search the web using the configured search provider with integrated semantic similarity matching. Supports single or multiple queries with configurable results per query. For better results, use specific, domain-relevant terms and combine multiple related queries to capture different perspectives on the same topic. Returns provider search results enhanced with semantically relevant content chunks from discovered pages when available. Each result includes normalized search data plus similarity-matched content snippets that directly answer your query. If no similarity matches exist for a page, only the original snippet is returned. This eliminates the need for sequential web.search + web.readFromPage operations by providing both discovery and content extraction in a single response.',
  READ_FROM_PAGE:
